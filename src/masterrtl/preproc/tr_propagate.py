...
from masterrtl.vlg2ir.directed_graph_node import DirectedGraphNode as Node
    ...
    if node.type in ["Operator", "UnaryOperator"]:
        ...
        if op == "Concat":
            ...
            if len(switch_prop_lst) == 2:
                ...
                t1 = (
                    switch_prop_lst[0] * switch_prop_lst[1]
                    + (1 - switch_prop_lst[0]) * switch_prop_lst[2]
                )
            tc = t1 * (1 - t1)
        elif op in ["And"]:
            t1 = 1
            ...
            t1 = switch_prop_lst[0] * (1 - switch_prop_lst[1]) + switch_prop_lst[1] * (
                1 - switch_prop_lst[0]
            )
            tc = t1 * (1 - t1)
...
def update_node_dict(g: DirectedGraph):
