...
def run_one_design(design_name, cmd, out_path):
    with open(f"../../example/feature/{design_name}_{cmd}_vec_area.json") as f:
        feat_vec = json.load(f)

    with open(f"../../example/verilog/toggle_rate/{design_name}_tc_sum_all.json") as f:
        tr_sum = json.load(f)
        feat_vec.append(tr_sum)

    with open(f"../../example/verilog/toggle_rate/{design_name}_tc_avr_all.json") as f:
        tr_avr = json.load(f)
