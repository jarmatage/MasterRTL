...
from masterrtl.pyverilog.utils.op2mark import op2mark, op2order
...
class ASTCodeGenerator(ConvertVisitor):
    ...
    def visit_Operator(self, node):
        ...
        if (
            not isinstance(
                node.left,
                (Sll, Srl, Sra, LessThan, GreaterThan, LessEq, GreaterEq, Eq, NotEq, Eql, NotEql),
            )
    ...
    def visit_Case(self, node):
        ...
        template = self.get_template(filename)
        condlist = (
            ["default"] if node.cond is None else [del_paren(self.visit(c)) for c in node.cond]
        )
        cond = []
        for c in condlist:
            cond.append(c)
