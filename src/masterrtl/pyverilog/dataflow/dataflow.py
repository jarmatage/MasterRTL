...
import masterrtl.pyverilog.utils.op2mark as op2mark
...
class Term:
    ...
    def tostr(self):
        ...
        if self.dims is not None:
            ret += " dims:"
            ret += "".join(["[" + l.tostr() + ":" + r.tostr() + "]" for l, r in self.dims])
        ret += ")"
        return ret
    ...
    def tocode(self):
        ...
        if self.dims is not None:
            code += "".join(["[" + l.tocode() + ":" + r.tocode() + "]" for l, r in self.dims])
        code += ";\n"
        return code
