...
def map_key(f, d):
    return collections.OrderedDict([(f(k), v) for k, v in d.items()])


def map_value(f, d):
    return collections.OrderedDict([(k, f(v)) for k, v in d.items()])
...
class Frame:
    ...
    def setAlwaysInfo(
        self, clock_name, clock_edge, clock_bit, reset_name, reset_edge, reset_bit, senslist
    ):
        self.alwaysinfo = AlwaysInfo(
    ...
    def setBlockingAssign(self, dst, bind):
        if dst not in self.blockingassign:
    ...
    def addNonblockingAssign(self, dst, bind):
        if dst not in self.nonblockingassign:
...
class FrameTable:
    ...
    def setAlwaysInfo(
        ...
        self.dict[self.current].setAlwaysInfo(
            clock_name, clock_edge, clock_bit, reset_name, reset_edge, reset_bit, senslist
        )
    ...
    def getAllSignals(self):
        ...
        for dk, dv in self.dict.items():
            ret.update(map_key((lambda x: dk + ScopeLabel(x, "signal")), dv.getSignals()))
        return ret

    def getAllConsts(self):
        ret = collections.OrderedDict()
        for dk, dv in self.dict.items():
            ret.update(map_key((lambda x: dk + ScopeLabel(x, "signal")), dv.getConsts()))
        return ret

    def getAllFunctions(self):
        ret = collections.OrderedDict()
        for dk, dv in self.dict.items():
            ret.update(map_key((lambda x: dk + ScopeLabel(x, "function")), dv.getFunctions()))
        return ret

    def getAllTasks(self):
        ret = collections.OrderedDict()
        for dk, dv in self.dict.items():
            ret.update(map_key((lambda x: dk + ScopeLabel(x, "task")), dv.getTasks()))
        return ret

    def getSignals(self, scope):
        return map_key((lambda x: scope + ScopeLabel(x, "signal")), self.dict[scope].getSignals())

    def getConsts(self, scope):
        return map_key((lambda x: scope + ScopeLabel(x, "signal")), self.dict[scope].getConsts())
    ...
    def getTasks(self, scope):
        return map_key((lambda x: scope + ScopeLabel(x, "task")), self.dict[scope].getTasks())
    ...
    def searchMatchedScopeChain(self, currentchain, targetchain):
        ...
        if currentchain[skiplength].scopetype == "for" and targetchain[0].scopetype == "for":
