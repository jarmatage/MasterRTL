...
class VerilogOptimizer:
            ...
            if len(usednodes) == 1:
                return DFConcat(
                    (
                        ...
                        DFPartselect(
                            usednodes[0], DFEvalValue(widsum - 1), DFEvalValue(lsb + lsboffset)
                        ),
                    )
                )
            return DFConcat(
    ...
    def replaceOperator(self, node):
        ...
        if node.operator == "Times" and (
            ...
            return DFOperator(
                (node.nextnodes[0], DFEvalValue(int(math.log(node.nextnodes[1].value, 2)))), "Sll"
            )
        if node.operator == "Times" and (
            ...
            return DFOperator(
                (node.nextnodes[1], DFEvalValue(int(math.log(node.nextnodes[0].value, 2)))), "Sll"
            )
        if node.operator == "Divide" and (
            ...
            return DFOperator(
                (node.nextnodes[0], DFEvalValue(int(math.log(node.nextnodes[1].value, 2)))), "Sra"
            )
        return node
