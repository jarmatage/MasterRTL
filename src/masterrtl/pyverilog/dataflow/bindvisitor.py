...
import masterrtl.pyverilog.dataflow.reorder as reorder
import masterrtl.pyverilog.dataflow.replace as replace
import masterrtl.pyverilog.utils.signaltype as signaltype
import masterrtl.pyverilog.utils.verror as verror
from masterrtl.pyverilog.dataflow.dataflow import *
from masterrtl.pyverilog.dataflow.optimizer import VerilogOptimizer
from masterrtl.pyverilog.dataflow.visit import *
from masterrtl.pyverilog.utils.scope import ScopeChain, ScopeLabel
...
class BindVisitor(NodeVisitor):
    ...
    def _visit_Instance_body(self, node, nodename, arrayindex=None):
        ...
        for paramnames_i, param in enumerate(node.parameterlist):
            ...
            if paramname not in paramnames:
                raise verror.FormatError("No such parameter: %s in %s" % (paramname, nodename))
        ...
        for ioport_i, port in enumerate(node.portlist):
            if port.portname is not None and port.portname not in ioports:
                raise verror.FormatError("No such port: %s in %s" % (port.argname.name, nodename))
    ...
    def _visit_Instance_primitive(self, node, arrayindex=None):
        ...
        if primitive_type == None:
            ...
            concat_list = (
                ...
                if arrayindex is None
                else [Pointer(p.argname, IntConst(str(arrayindex))) for p in node.portlist[1:]]
            )
    ...
    def visit_Always(self, node):
        ...
        (clock_name, clock_edge, clock_bit, reset_name, reset_edge, reset_bit, senslist) = (
    ...
    def visit_NonblockingSubstitution(self, node):
        if self.frames.isForpre() or self.frames.isForpost():
            raise verror.FormatError("Non Blocking Substitution is not allowedin for-statement")
    ...
    def makeDFTree(self, node, scope):
        ...
        raise verror.FormatError("unsupported AST node type: %s %s" % (str(type(node)), str(node)))
    ...
    def removeOverwrappedCondition(self, tree, current_bindlist, scope):
        ...
        condlist, flowlist = self.getCondflow(scope)
        (merged_tree, rest_condlist, rest_flowlist, match_flowlist) = self.diffBranchTree(
            merged_tree, condlist, flowlist
        )
    ...
    def resolveBlockingAssign(self, tree, scope):
        ...
        if isinstance(tree, DFPartselect):
            ...
            if isinstance(resolved_var, DFBranch):
                return reorder.insertPartselect(resolved_var, resolved_msb, resolved_lsb)
            return DFPartselect(resolved_var, resolved_msb, resolved_lsb)

        if isinstance(tree, DFPointer):
            resolved_ptr = self.resolveBlockingAssign(tree.ptr, scope)
            if isinstance(tree.var, DFTerminal) and self.getTermDims(tree.var.name) is not None:
                ...
                print("Warning: Overwrting Blocking Assignment with Reg Array is not supported")
                return new_tree
            resolved_var = self.resolveBlockingAssign(tree.var, scope)
            if isinstance(resolved_var, DFBranch):
        ...
        raise verror.FormatError("unsupported DFNode type: %s %s" % (str(type(tree)), str(tree)))
    ...
    def getMergedTree(self, bindlist):
        ...
        def bindkey(x):
            ...
            length = abs(self.optimize(term.msb).value - self.optimize(term.lsb).value) + 1
    ...
    def getDestinations(self, left, scope):
        ...
        for name, msb, lsb, ptr in reversed(dst):
            ...
            part_offset = reorder.reorder(DFOperator((part_msb, DFIntConst("1")), "Plus"))
    ...
    def getDst(self, left, scope):
        ...
        raise verror.FormatError("unsupported AST node type: %s %s" % (str(type(left)), str(left)))

    def setDataflow(self, dst, raw_tree, condlist, flowlist, alwaysinfo=None, bindtype=None):
    ...
    def setDataflow_rename(self, dst, raw_tree, condlist, flowlist, scope, alwaysinfo=None):
    ...
    def setRenamedFlow(self, dst, renamed_dst, condlist, flowlist, scope, alwaysinfo=None):
        term_i = 0
        for name, msb, lsb, ptr, part_msb, part_lsb in dst:
    ...
    def makeBind(
        ...
        if len(current_bindlist) > 0:
            for current_bind in current_bindlist:
                if current_bind.msb == msb and current_bind.lsb == lsb and current_bind.ptr == ptr:
        ...
        if current_msb == msb and current_lsb == lsb and current_ptr == ptr:
            (rest_tree, rest_condlist, rest_flowlist, match_flowlist) = self.diffBranchTree(
                current_tree, condlist, flowlist
            )
        ...
        tree = reorder.reorder(self.appendBranchTree(current_tree, match_flowlist, add_tree))
    ...
    def diffBranchTree(self, tree, condlist, flowlist, matchflowlist=()):
        ...
        if flowlist[0]:
            return self.diffBranchTree(
                tree.truenode, condlist[1:], flowlist[1:], matchflowlist + (flowlist[0],)
            )
        else:
            return self.diffBranchTree(
                tree.falsenode, condlist[1:], flowlist[1:], matchflowlist + (flowlist[0],)
            )
    ...
    def makeBranchTree(self, condlist, flowlist, node):
        ...
        if len(condlist) == 1:
            ...
            if flowlist[0]:
                return DFBranch(
                    condlist[0], self.makeBranchTree(condlist[1:], flowlist[1:], node), None
                )
            else:
                return DFBranch(
                    condlist[0], None, self.makeBranchTree(condlist[1:], flowlist[1:], node)
                )
