...
def insertBranch(base, truenode, falsenode):
    if isinstance(base, DFBranch):
        return DFBranch(
            base.condnode,
            insertBranch(base.truenode, truenode, falsenode),
...
def insertUnaryOp(base, op):
    if isinstance(base, DFBranch):
        return DFBranch(
            base.condnode, insertUnaryOp(base.truenode, op), insertUnaryOp(base.falsenode, op)
        )
...
def insertOp(left, right, op):
    if isinstance(left, DFBranch):
        return DFBranch(
            left.condnode, insertOp(left.truenode, right, op), insertOp(left.falsenode, right, op)
        )
    elif isinstance(right, DFBranch):
        return DFBranch(
            right.condnode, insertOp(left, right.truenode, op), insertOp(left, right.falsenode, op)
        )
...
def insertPartselect(var, msb, lsb):
    if isinstance(var, DFBranch):
        return DFBranch(
            var.condnode,
            insertPartselect(var.truenode, msb, lsb),
            insertPartselect(var.falsenode, msb, lsb),
        )
    if var is None:
        return None
...
def insertPointer(var, ptr):
    if isinstance(var, DFBranch):
        return DFBranch(
            var.condnode, insertPointer(var.truenode, ptr), insertPointer(var.falsenode, ptr)
        )
    if var is None:
        return None
