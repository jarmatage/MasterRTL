...
class VerilogDataflowMerge:
    ...
    def getBindlist(self, termname):
        if termname not in self.binddict:
            return ()
        return self.binddict[termname]

    def getResolvedTerm(self, termname):
        if termname not in self.resolved_terms:
            return None
        ...
        if self.getTermDims(termname) is not None:
            ...
            for bind in bindlist:
                if isinstance(bind.ptr, DFEvalValue):
                    ...
                    if ptrval not in discretebinds:
                        discretebinds[ptrval] = []
                    discretebinds[ptrval] += [bind]
                else:
                    if "any" not in discretebinds:
                        discretebinds["any"] = []
    ...
    def splitBindlist(self, bindlist, split_positions):
        if len(bindlist) == 0:
            return ()
        return self.splitBindPositions(bindlist[0], split_positions) + self.splitBindlist(
    ...
    def splitBind(self, bind, splitpos):
        ...
        if splitpos > lsb.value and splitpos <= msb.value:  # split
            ...
            right_tree = reorder.reorder(
                DFPartselect(copy.deepcopy(tree), DFEvalValue(right_width - 1), DFEvalValue(0))
            )
