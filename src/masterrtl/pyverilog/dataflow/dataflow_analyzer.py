...
from masterrtl.pyverilog.dataflow.bindvisitor import BindVisitor
from masterrtl.pyverilog.dataflow.modulevisitor import ModuleVisitor
from masterrtl.pyverilog.dataflow.signalvisitor import SignalVisitor
from masterrtl.pyverilog.vparser.parser import VerilogCodeParser
...
class VerilogDataflowAnalyzer(VerilogCodeParser):
    def __init__(
        ...
        files = (
            filelist if isinstance(filelist, tuple) or isinstance(filelist, list) else [filelist]
        )
        VerilogCodeParser.__init__(
            self, files, preprocess_include=preprocess_include, preprocess_define=preprocess_define
        )
    ...
    def generate(self):
        ...
        bind_visitor = BindVisitor(
            moduleinfotable, self.topmodule, frametable, noreorder=self.noreorder
        )
