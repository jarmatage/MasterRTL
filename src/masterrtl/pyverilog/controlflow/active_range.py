...
import masterrtl.pyverilog.utils.inference as inference
import masterrtl.pyverilog.utils.signaltype as signaltype
import masterrtl.pyverilog.utils.util as util
from masterrtl.pyverilog.controlflow.controlflow_analyzer import VerilogControlflowAnalyzer
...
class VerilogActiveAnalyzer(VerilogControlflowAnalyzer):
    ...
    def getActiveConditions(self, termname, op=">", conditionvalue=0):
        if termname not in self.resolved_binddict:
            return ()
    ...
    def inferActiveConditions(self, condlists):
        ...
        for condlist in condlists:
            ...
            if isinstance(l, ActiveTerm):
                ...
                if activecond not in active_conditions:
                    active_conditions.append(activecond)
            if isinstance(l, ActiveCondition):
                l.resolve()
                if len(l) > 0 and l not in active_conditions:
    ...
    def toTree(self):
        ...
        for rmin, rmax in self.range_pairs:
            if rmax is None:
                ...
                t = DFOperator(
                    (
                        DFOperator((DFEvalValue(rmin), DFTerminal(self.name)), "LessEq"),
                        DFOperator((DFTerminal(self.name), DFEvalValue(rmax)), "GreaterEq"),
                    ),
                    "Land",
                )
                if retnode is None:
...
class ActiveCondition:
    ...
    def satisfy(self, valuedict):
        for termname, value in valuedict.items():
            if termname not in self.termdict:
                continue
