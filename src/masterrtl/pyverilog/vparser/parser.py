...
from masterrtl.pyverilog.vparser.lexer import VerilogLexer
from masterrtl.pyverilog.vparser.preprocessor import VerilogPreprocessor
...
class VerilogParser:
    ...
    def p_param_signed(self, p):
        ...
        paramlist = [
            Parameter(rname, rvalue, signed=True, lineno=p.lineno(2)) for rname, rvalue in p[3]
        ]
        p[0] = Decl(tuple(paramlist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
    ...
    def create_ioport(self, sigtypes, name, width=None, dimensions=None, lineno=0):
        ...
        if "input" in sigtypes:
            first = Input(
                name=name, width=width, signed=signed, dimensions=dimensions, lineno=lineno
            )
    ...
    def create_decl(self, sigtypes, name, width=None, dimensions=None, lineno=0):
        ...
        if "input" in sigtypes:
            decls.append(
                Input(name=name, width=width, signed=signed, lineno=lineno, dimensions=dimensions)
            )
        if "output" in sigtypes:
            decls.append(
                Output(name=name, width=width, signed=signed, lineno=lineno, dimensions=dimensions)
            )
        if "inout" in sigtypes:
            decls.append(
                Inout(name=name, width=width, signed=signed, lineno=lineno, dimensions=dimensions)
            )
        if "wire" in sigtypes:
            decls.append(
                Wire(name=name, width=width, signed=signed, lineno=lineno, dimensions=dimensions)
            )
        if "reg" in sigtypes:
            decls.append(
                Reg(name=name, width=width, signed=signed, lineno=lineno, dimensions=dimensions)
            )
        if "tri" in sigtypes:
            decls.append(
                Tri(name=name, width=width, signed=signed, lineno=lineno, dimensions=dimensions)
            )
    ...
    def p_decl(self, p):
        ...
        for rname, rdimensions in p[2]:
            decllist.extend(
                self.create_decl(p[1], rname, dimensions=rdimensions, lineno=p.lineno(2))
            )
    ...
    def p_localparamdecl_signed(self, p):
        ...
        paramlist = [
            Localparam(rname, rvalue, signed=True, lineno=p.lineno(2)) for rname, rvalue in p[3]
        ]
        p[0] = Decl(tuple(paramlist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
    ...
    def p_instance(self, p):
        ...
        for instance_name, instance_ports, instance_array in p[3]:
            instancelist.append(
                Instance(
                    p[1], instance_name, instance_ports, p[2], instance_array, lineno=p.lineno(1)
                )
            )
    ...
    def p_instance_noname(self, p):
        ...
        for instance_name, instance_ports, instance_array in p[2]:
            instancelist.append(
                Instance(
                    p[1], instance_name, instance_ports, (), instance_array, lineno=p.lineno(1)
                )
            )
    ...
    def p_scope_pointer(self, p):
        ...
        p[0] = IdentifierScope(
            scope + (IdentifierScopeLabel(p[1].var.name, p[1].ptr, lineno=p.lineno(1)),),
            lineno=p.lineno(1),
        )
        p.set_lineno(0, p.lineno(1))
...
def parse(filelist, preprocess_include=None, preprocess_define=None, outputdir=".", debug=True):
