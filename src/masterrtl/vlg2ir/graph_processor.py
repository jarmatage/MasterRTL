    ...
    for node_name in path:
        ...
        if node_type in ["Operator", "Mux", "UnaryOperator"]:
            ...
            if op in ["Mux", "Cond"]:
                ...
                num_xor += 1
    path_feat_vec = [path_delay, path_len, num_mux, num_and, num_or, num_not, num_xor, 0]
    ...
    def Graph_STA(self, rfr, design_name):
        ...
        for node in topo_sort:
            ...
            for successor in g_nx.successors(node):
                ...
                self.g.node_dict[successor].update_AT(
                    self.g.node_dict[node].AT, self.g.node_dict[node].path, visited
                )
